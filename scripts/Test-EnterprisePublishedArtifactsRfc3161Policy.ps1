...
function Test-RealRfc3161Fixture {
    ...
    foreach ($candidate in $candidates) {
        ...
        try {
            ...
            Write-Output "ENTERPRISE-RFC3161-REAL-FIXTURE-PASS: $candidate"
            return
        }
        catch {
    ...
    if (-not [string]::IsNullOrWhiteSpace($ExplicitPath)) {
        throw 'The explicitly supplied Enterprise RFC3161 fixture was not admitted: ' +
            ($diagnostics -join ' | ')
    }
    Write-Output ('ENTERPRISE-RFC3161-REAL-FIXTURE-PENDING: ' +
        ($diagnostics -join ' | '))
}

$codeCertificate = New-TestSigningCertificate `
    -Subject 'CN=Ensou Enterprise RFC3161 code fixture'
$timestampCertificate = New-TestSigningCertificate `
    -Subject 'CN=Ensou Enterprise RFC3161 TSA fixture' `
    -Timestamping
try {
    ...
    if ([string]$validAdmission.TimestampProtocol -cne 'RFC3161') {
        throw 'Synthetic Enterprise RFC3161 positive did not pass canonical admission.'
    }
    ...
    if ($null -eq $alignmentCms) {
        throw 'Could not create an Enterprise WIN_CERTIFICATE alignment fixture.'
    }
    ...
    if ($alignmentPadding -le 0) {
        throw 'Enterprise truncated-alignment fixture unexpectedly has no WIN_CERTIFICATE padding.'
    }
    ...
    [BitConverter]::GetBytes([uint32]$certificateLength).CopyTo(
        ...
        0x80 + 24 + 96 + 32 + 4)
    Assert-Throws `
        -Label 'Enterprise truncated WIN_CERTIFICATE alignment regression' `
    ...
    Assert-Throws `
        -Label 'Enterprise wrong-primary-SignerInfo RFC3161 token regression' `
    ...
    Assert-Throws `
        -Label 'Enterprise invalid primary CMS with bound RFC3161 token regression' `
    ...
    Assert-Throws `
        -Label 'Enterprise Authenticode locked-PE digest regression' `
    ...
    Assert-Throws `
        -Label 'Enterprise Authenticode section-layout digest regression' `
        -ExpectedMessage 'digest differs from the locked PE bytes' `
        -Action {
            [void](ProductionReleaseState\Assert-PeRfc3161Timestamp `
                -Bytes (New-TestPeWithCms `
    ...
    Assert-Throws `
        -Label 'Enterprise post-certificate overlay digest regression' `
        -ExpectedMessage 'digest differs from the locked PE bytes' `
        -Action {
            [void](ProductionReleaseState\Assert-PeRfc3161Timestamp `
    ...
    Assert-Throws `
        -Label 'Enterprise non-Authenticode CMS content-type regression' `
    ...
    Assert-Throws `
        -Label 'Enterprise invalid auxiliary CMS RFC3161 regression' `
        -ExpectedMessage 'CMS signature is invalid' `
        -Action {
            [void](ProductionReleaseState\Assert-PeRfc3161Timestamp `
                -Bytes (New-TestPeWithCms `
    ...
    Assert-Throws `
        -Label 'Enterprise legacy counterSignature regression' `
    ...
    Write-Output 'ENTERPRISE-RFC3161-NEGATIVE-CONTRACTS-PASS'
    Test-RealRfc3161Fixture -ExplicitPath $RealRfc3161PeFixturePath
    Write-Output 'ENTERPRISE-RFC3161-POLICY-CONTRACT-PASS'
}
finally {
